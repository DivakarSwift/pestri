...
class PauseView: UIView {
    ...
    func setup() {
        ...
        backButton.addTarget(scene, action: Selector("abortGame"), forControlEvents: UIControlEvents.TouchUpInside)
        self.addSubview(backButton)
        
        continueButton = UIButton(frame: CGRect(x: width / 2 - buttonWidth / 2, y: height / 4 - buttonHeight / 2,
            width: buttonWidth, height: buttonHeight))
        continueButton.setTitle("Continue", forState: .Normal)
        continueButton.addTarget(scene, action: Selector("continueGame"), forControlEvents: UIControlEvents.TouchUpInside)
